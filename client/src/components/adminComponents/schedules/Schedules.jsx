...
const Schedules = () => {
  ...
  const scheduleByBarber = async (barberId) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/schedule-by-barber?period=${period}&barberId=${barberId}`, {
        method: 'get',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  return (
    <>
      <Box className='admin-scheduler-container'>
        ...
        {period && <Box display={'flex'} gap={2}>
          {barbers?.map((barber) => (
            <div
              className={`btn-choose ${selectedBarber?.id === barber.id && 'active'}`}
              key={barber.id}
              onClick={() => { handleBarber(barber) }}
            >
              {barber.name} {barber.last_name}
            </div>
          ))}
        </Box>}
        {selectedBarber && emptyDates && <AddSchedule period={period} barberId={selectedBarber.id} />}
        {selectedBarber && !emptyDates && <ManageSchedules barberId={selectedBarber.id} selectedMonth={selectedMonth} />}
      </Box>
    </>
  )
}
