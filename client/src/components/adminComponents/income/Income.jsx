...
import { Accordion, AccordionDetails, AccordionSummary, Box, Button, MenuItem, OutlinedInput, Select, Table, TableBody, TableCell, TableHead, TableRow, Typography } from '@mui/material'
...
const Income = () => {
  ...
  const getData = async (year, month) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/incomes?year=${year}&month=${month}`, {
        method: 'get',
        headers: {
  ...
  return (
    <>
      <Box className='income-search-container'>
        <Typography component='span' sx={{ color: 'text.secondary' }}>Ver datos para el mes de</Typography>
        <Select
        ...
        <Typography component='span' sx={{ color: 'text.secondary' }}>del año</Typography>
        <Select
        ...
        <Button variant="contained" onClick={() => getData(selectedYear, selectedMonth)}>
          Buscar
        </Button>
      </Box>
      {barbers !== null && Object.entries(barbers).map(([id, data]) => {
        return (
      ...
      {barbers !== null &&
        Object.entries(barbers).length === 0 &&
        <Typography sx={{ mt: 8, color: 'text.secondary' }}>No hay datos registrados para esta fecha</Typography>}
    </>
  )
}
