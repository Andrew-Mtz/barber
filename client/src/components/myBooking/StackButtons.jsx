...
const StackButtons = ({
  ...
  const getLastsBooking = async () => {
    try {
      ...
      setLoading(false);
      setInfoMessage('Ocurrió un error al obtener las reservas.');
    }
  };
  ...
  return (
    <Stack direction="row" spacing={2} sx={{ alignSelf: 'flex-start' }}>
      <Box>
        <Button sx={buttonStyles.btnActive} onClick={() => getMyBooking()}>
          Mi reserva
        </Button>
      </Box>
      <Box>
        <Button sx={buttonStyles.btnInactive} onClick={() => getLastsBooking()}>
          Reservas pasadas
        </Button>
      </Box>
    </Stack>
...
StackButtons.propTypes = {
  setBooking: PropTypes.func,
  setInfoMessage: PropTypes.func,
  setMyReservation: PropTypes.func,
  myReservation: PropTypes.bool,
