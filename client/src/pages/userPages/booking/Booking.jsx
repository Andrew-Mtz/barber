...
const Booking = () => {
  ...
  const isAccountRoute =
    location.state && location.state.previousPath === '/account';
  ...
  React.useEffect(() => {
    if (isAccountRoute) {
      ...
      return storedData
        ? (setActiveStep(2), setBookingData(JSON.parse(storedData)))
        : setBookingData({
            status: 'scheduled',
            barber_id: 0,
            barber_name: '',
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 409) {
          ...
          setErMsgBooking(
            'Puedes cancelar tu reserva hasta 2 horas antes en "Mi reserva" y luego agendar una nueva',
          );
        ...
        setErMsgBooking(
          'Ha ocurrido un error inesperado, espera unos minutos e intenta de nuevo. Si el error persiste hasnos saber!',
        );
        setActiveStep(prevActiveStep => prevActiveStep + 1);
        return setSucces(false);
      }
  ...
  const stepComponents = [
    <BarberList
      selectedId={bookingData.barber_id}
      onBarberSelect={handleBarberSelect}
    />,
    <HaircutList
      selectedId={bookingData.haircut_id}
      selectedBarberId={bookingData.barber_id}
      onHaircutSelect={handleHaircutSelect}
    />,
    <Calendar
      selectedBarberId={bookingData.barber_id}
      selectedId={bookingData.schedule_id}
      selectedDateId={bookingData.date_id}
      ...
      {activeStep === steps.length ? (
          ...
          >
            {succes ? (
              <SuccesfullMessage
                ...
                message={
                  'Puedes ver los detalles de la reserva o cancelarla hasta 2 horas antes en "Mi reserva"'
                }
              />
            ) : (
        ...
        <>
          ...
          <DialogConfirm
            ...
            message={
              'Para completar la reserva debes tener una cuenta. Presione en continuar para crear una cuenta o iniciar sesión si ya tiene una. Los datos seleccionados quedaran guardados '
            }
          />
        </>
      )}
