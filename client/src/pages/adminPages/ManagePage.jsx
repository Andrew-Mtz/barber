    ...
    <Box className="container-control-panel">
      <nav>
        <Box className="logo-name">
          <Typography component="span" className="logo_name">
            Panel administrador
          </Typography>
        </Box>
          ...
          <ul className="nav-links">
            {cpMenu.map((item, index) => (
              <li key={index} onClick={() => setSecTitle(item.title)}>
                <Typography>
                  <item.icon
                    className={`icon ${secTitle === item.title && 'active'}`}
                  />
                  <Typography
                    component="span"
                    className={`link-name ${secTitle === item.title && 'active'}`}
                  >
                    {item.title}
                  </Typography>
                </Typography>
          ...
          <ul className="logout-mode">
            <li onClick={logOut}>
              <Typography>
                ...
                <Typography component="span" className="link-name">
                  Cerrar sesion
                </Typography>
              </Typography>
            </li>
      ...
      <Box component="section" className="cp-section">
        ...
        {cpBody?.map((item, index) => (
          <Box key={index}>{item.title === secTitle && <item.component />}</Box>
        ))}
      </Box>
    </Box>
  );
